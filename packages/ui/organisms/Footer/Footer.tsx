  ...
  return (
    <footer>
      <div className="flex flex-col items-center gap-y-20 bg-white p-20 lg:flex-row lg:justify-between lg:p-40">
        {contact ? <a
          className="flex items-center gap-x-12 lg:ms-auto"
          href={`tel:${contact.phoneNumber.split(" ").join("")}`}
        >
            ...
            <div className="divide-sand divide-y md:flex md:flex-wrap md:items-start md:gap-x-32 md:divide-none">
              {columns.map(({label, links}) => (
                <div key={label} className="py-20 md:pt-0">
                  <FooterColumn
                    label={label}
                    header={<span className="text-b3 font-semibold">{label}</span>}
                  >
...
const FooterColumn: FunctionComponent<PropsWithChildren<{ header: ReactNode; label?: string }>> = ({
                                                                                                     header,
                                                                                                     children,
                                                                                                     label = ""
                                                                                                   }) => {
