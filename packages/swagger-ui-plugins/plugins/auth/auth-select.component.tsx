    ...
    <RadioGroup className={"mt-16"} value={auths.keySeq().first()} onChange={onChange}>
      {auths
        ...
        .map((value, connection) => {
          const profile = decodeToken(value?.getIn(["token", "id_token"]) as string);
            ...
            <Radio key={connection} className="mb-16" value={connection}>
              <span className={"flex flex-col"}>
                <span className="block text-b4">
                  {cmAuthName} - {profile.given_name} {profile.family_name}
                </span>
                <small className="text-b5 font-normal">
                  {cmAuthName === "GO" ? profile.clubmed_id : cmAuthName === "Partner" ? profile.partner_id : profile.sub}
                </small>
